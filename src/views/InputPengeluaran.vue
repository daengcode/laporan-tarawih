        ...
        <div class="p-4 space-y-6">
          ...
          <section class="space-y-4">
            ...
            <div class="space-y-3">
              <!-- Tombol Tambah -->
              <button
                @click="tambahPengeluaranLainnya"
                class="w-full flex items-center gap-2 p-3 rounded-xl border-2 border-dashed border-orange-200 dark:border-orange-800 hover:border-orange-400 hover:bg-orange-50 transition-all"
              >
                <span class="material-symbols-outlined text-orange-500">add</span>
                <span class="text-sm font-semibold text-orange-700 dark:text-orange-300"
                  >Tambah Pengeluaran</span
                >
              </button>
            </div>
  ...
  if (ceramah > 0) {
    ...
    if (!result.success) {
      Swal.fire({
        icon: "error",
        title: "Gagal",
        text: result.error || "Gagal menyimpan honor ceramah!",
        confirmButtonColor: "#13ec80",
      });
      return;
    }
  ...
  if (imam > 0) {
    ...
    if (!result.success) {
      Swal.fire({
        icon: "error",
        title: "Gagal",
        text: result.error || "Gagal menyimpan honor imam!",
        confirmButtonColor: "#13ec80",
      });
      return;
    }
      ...
      if (biaya > 0) {
        ...
        if (!result.success) {
          Swal.fire({
            icon: "error",
            title: "Gagal",
            text: result.error || "Gagal menyimpan pengeluaran lainnya!",
            confirmButtonColor: "#13ec80",
          });
          return;
        }
