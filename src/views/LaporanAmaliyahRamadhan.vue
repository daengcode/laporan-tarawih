...
const { getTransaksi, getTransaksiByDate, getTransaksiBeforeDate } = useLaporan();
...
const currentIncome = computed(() => {
  if (!selectedDateTransactions.value || selectedDateTransactions.value.length === 0) return 0;
...
const selectDate = (date) => {
  ...
  updateSelectedDateTransactions(date.date);
};
...
const groupTransactionsByDate = () => {
  ...
  if (dates.value.length > 0) {
    ...
    updateSelectedDateTransactions(dates.value[0].date);
  } else {
