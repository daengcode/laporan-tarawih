<template>
  <div class="bg-background-light dark:bg-background-dark min-h-screen pb-24">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-primary/5 overflow-hidden">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr
              v-for="(ceramah, index) in daftarCeramah"
              :key="index"
              :class="[
                'transition-colors',
                ceramah.hari === currentRamadhanDay
                  ? 'bg-primary/20 font-bold text-primary'
                  : 'hover:bg-gray-50',
              ]"
            >
              <td class="px-4 py-3 text-sm text-gray-800">{{ ceramah.hari }}</td>
              <td class="px-4 py-3 text-sm text-gray-800">{{ ceramah.tanggal }}</td>
              <td class="px-4 py-3 text-sm text-gray-800">{{ ceramah.nama }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <BottomMenu current-page="ceramah" @open-modal="showAddModal = true" />
  </div>
</template>
...
const daftarCeramah = Array.from({ length: 30 }, (_, index) => ({
  hari: index + 1,
